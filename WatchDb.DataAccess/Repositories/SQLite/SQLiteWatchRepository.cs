...
{
    public class SQLiteWatchRepository : IWatchRepository
    {
        private DBConfig configuration;

        public SQLiteWatchRepository(DBConfig configuration)
        {
        ...
        public async Task<WatchModel> CreateAsync(WatchModel watch)
        {
            ...
            int id = await connection.QueryFirstOrDefaultAsync<int>("insert into Watches(CategoryId, ProducerId, Title, Price, Available, OnSale, Image) values(@CategoryId, @ProducerId, @Title, @Price, @Available, @OnSale, @Image); select Id from Watches order by Id desc limit 1;", watch);
            watch.Id = id;
            return watch;
        }
        ...
        {
            using IDbConnection connection = new SqliteConnection(configuration.ConnectionString);
            return await connection.ExecuteAsync("delete from Watches where Id = @Id", new { Id = id }) != 0;
        }
